...
window.jQuery = function (selectorOrArrayOrTemplate) {
  ...
  return api = {
    jquery: true,
    elements: elements,
    get: function get(index) //获取指定下标的元素
    {
      return elements[index];
    },
    appendTo: function appendTo(node) //将API添加到指定父亲中
    {
    ...
    append: function append(children) //向API中添加一个孩子
    {
      var _this = this;
    ...
    find: function find(selector) //查找API中所有的selector元素
    {
      var array = [];

      for (var i = 0; i < elements.length; i++) {
        var elements2 = Array.from(elements[i].querySelectorAll(selector)); //因为querySelectorAll返回的是一个伪数组，所以这里我们需要将它转化为数组
